...
async function main() {
  ...
  try {
    ...
    assert(/side-group-label/.test(adminHtml) && /تخفیف مبلغ سفارش/.test(adminHtml), "sidebar groups");
    assert(/min-height:50px/.test(adminHtml), "sidebar row height");
    ok("sidebar UI markup");
